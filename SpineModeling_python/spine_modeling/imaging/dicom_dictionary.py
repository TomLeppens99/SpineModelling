"""
DICOM Dictionary Module

This module provides a comprehensive mapping of DICOM tags to their descriptions
and value representations (VR). The dictionary maps hexadecimal tag strings to
VR+description strings for use in DICOM parsing.

DICOM Tag Format:
    Tags are 8-character hexadecimal strings (e.g., "00020002")
    Values are formatted as: "{VR}{Description}"
    Where VR is the Value Representation (UI, SH, DA, etc.)

Example Usage:
    >>> dictionary = DicomDictionary()
    >>> tag_info = dictionary.get_tag("00100010")
    >>> print(tag_info)
    'PNPatient's Name'
    >>> vr = dictionary.get_vr("00100010")
    >>> print(vr)
    'PN'
    >>> description = dictionary.get_description("00100010")
    >>> print(description)
    "Patient's Name"
...
class DicomDictionary:
    """
    DICOM tag dictionary mapping hexadecimal tag strings to VR and descriptions.

    This class provides a comprehensive mapping of DICOM tags based on the
    DICOM standard. Each tag is stored as an 8-character hexadecimal string
    (e.g., "00100010") and maps to a string containing the Value Representation
    (VR) code followed by the description.

    Attributes:
        dict (Dict[str, str]): Dictionary mapping tag hex strings to VR+description

    """

    def __init__(self):
        """Initialize the DICOM dictionary with comprehensive tag mappings."""
    ...
    def get_tag(self, tag: str) -> Optional[str]:
        """
        Get the full tag information (VR + description) for a DICOM tag.

        Args:
            tag: 8-character hexadecimal tag string (e.g., "00100010")

        Returns:
            VR and description string (e.g., "PNPatient's Name"), or None if not found

        Example:
            >>> dictionary = DicomDictionary()
            >>> info = dictionary.get_tag("00100010")
            >>> print(info)
            PNPatient's Name

        """
        return self.dict.get(tag.upper())

    def get_vr(self, tag: str) -> Optional[str]:
        """
        Extract the Value Representation (VR) code from a DICOM tag.

        Args:
            tag: 8-character hexadecimal tag string (e.g., "00100010")

        Returns:
            2-character VR code (e.g., "PN"), or None if tag not found
        ...
        if tag_info and len(tag_info) >= 2:
            return tag_info[:2]
        return None

    def get_description(self, tag: str) -> Optional[str]:
        """
        Extract the description from a DICOM tag.

        Args:
            tag: 8-character hexadecimal tag string (e.g., "00100010")

        Returns:
            Tag description (e.g., "Patient's Name"), or None if tag not found

        Example:
            >>> dictionary = DicomDictionary()
            >>> desc = dictionary.get_description("00100010")
            >>> print(desc)
            Patient's Name
    ...
    def contains_tag(self, tag: str) -> bool:
        """
        Check if a DICOM tag exists in the dictionary.
        ...
        Example:
            >>> dictionary = DicomDictionary()
            >>> exists = dictionary.contains_tag("00100010")
            >>> print(exists)
            True
    ...
    def get_all_tags(self) -> list:
        """
        Get a list of all DICOM tags in the dictionary.

        Returns:
            List of 8-character hexadecimal tag strings

        Example:
            >>> dictionary = DicomDictionary()
            >>> tags = dictionary.get_all_tags()
            >>> print(len(tags))
            803
